...
*   **Étapes Séquentielles de la Pipeline :**
    ...
    2.  **Build Custom Docker Images :** Construction des images Docker pour chaque service, taguées de manière unique.
    3.  **Setup Kind Cluster :** Création d'un cluster Kubernetes éphémère et pré-chargement de toutes les images.
    4.  **Deploy Application to Kind :** Rendu du manifeste Kubernetes avec les bons tags et déploiement. La pipeline attend ensuite activement la fin du déploiement.
    5.  **Integration/E2E Tests :** Étape réservée pour les futurs tests d'intégration.
